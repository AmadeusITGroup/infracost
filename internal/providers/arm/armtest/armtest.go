...
func ResourceTestsForARMProject(t *testing.T, armProject ARMProject, usage schema.UsageMap, checks []testutil.ResourceCheck, ctxOptions ...func(ctx *config.RunContext)) {
	t.Run("ARM", func(t *testing.T) {
		resourceTestsForARMProject(t, "arm", armProject, usage, checks, ctxOptions...)
	})
}

func resourceTestsForARMProject(t *testing.T, pName string, armProject ARMProject, usage schema.UsageMap, checks []testutil.ResourceCheck, ctxOptions ...func(ctx *config.RunContext)) {
	t.Helper()
	...
	projects := loadResources(t, pName, armProject, runCtx, usage)
...
func GoldenFileResourceTestsWithOpts(t *testing.T, testName string, options *GoldenFileOptions, ctxOptions ...func(ctx *config.RunContext)) {
	t.Run("ARM", func(t *testing.T) {
		goldenFileResourceTestWithOpts(t, "arm", testName, options, ctxOptions...)
	})
}
...
func goldenFileResourceTestWithOpts(t *testing.T, pName string, testName string, options *GoldenFileOptions, ctxOptions ...func(ctx *config.RunContext)) {
	...
	os.WriteFile("/tmp/file", actual, 0666)
	testutil.AssertGoldenFile(t, goldenFilePath, actual)
}

func loadResources(t *testing.T, pName string, armProject ARMProject, runCtx *config.RunContext, usageData schema.UsageMap) []*schema.Project {
	t.Helper()
...
func createARMProject(t *testing.T, armProject ARMProject) string {
	...
	tfdir, err := CreateARMProject(tmpDir, armProject)
	require.NoError(t, err)
